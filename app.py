...
from db import save_analysis, get_user_history
...
def load_user_history():
    """Load user's analysis history from database."""
    if st.session_state.user_id:
        history = get_user_history(st.session_state.user_id)
        st.session_state.sentiment_history = [
            {
                'timestamp': datetime.strptime(item[0], '%Y-%m-%d %H:%M:%S'),
                'text': item[1],
                'sentiment': item[2],
                'confidence': item[3]
            }
        ...
        if text_input:
            if st.session_state.trained_model is None:
                ...
                with st.spinner('Analyzing sentiment...'):
                    ...
                    if analysis_result:
                        # Store result in history and database
                        result_entry = {
                        ...
                        st.session_state.sentiment_history.append(result_entry)
